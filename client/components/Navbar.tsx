    ...
    <header className="relative z-50" style={{ backgroundColor: '#8B0000' }}>
      ...
      {menuOpen && (
        <nav className="md:hidden border-t border-white/20 px-6 py-4 flex flex-col gap-4">
          {isClient && (
            <Link href="/client/dashboard" className={navLinkClass} style={{ fontFamily: 'Georgia, serif' }} onClick={() => setMenuOpen(false)}>Dashboard</Link>
          )}
          {!isAuthenticated && (
            <Link href="/" className={navLinkClass} style={{ fontFamily: 'Georgia, serif' }} onClick={() => setMenuOpen(false)}>Home</Link>
          )}

          <Link href="/advocates" className={navLinkClass} style={{ fontFamily: 'Georgia, serif' }} onClick={() => setMenuOpen(false)}>Advocate Listing</Link>
          ...
          {!isAuthenticated && (
            <>
              <Link href="/signup" className={navLinkClass} style={{ fontFamily: 'Georgia, serif' }} onClick={() => setMenuOpen(false)}>Sign Up</Link>
              <Link href="/login" className={navLinkClass} style={{ fontFamily: 'Georgia, serif' }} onClick={() => setMenuOpen(false)}>Login</Link>
            </>
          )}
