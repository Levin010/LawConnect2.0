...
export default function ChatPage({ otherUserId }: Props) {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      markAsRead(messages[0].conversationId);
    }
  ...
  // Get the other person's name from the first message
  const otherUserName = messages.find(m => m.senderId === otherUserId)?.senderName
    ?? 'Client';
          ...
          return (
            <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
              <div
                ...
                style={{
                  backgroundColor: isMine ? '#8B0000' : undefined,
